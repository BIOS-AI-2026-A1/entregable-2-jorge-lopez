...
from app.servicios import ensamblar_contenido
...
def _categoria(db, categoria_id: str, *, orden: int = 0, idiomas=("es", "pt")) -> None:
    db.add(Categoria(id=categoria_id, icono="i", fondo="bg", texto="tx", orden=orden))
    for idioma in idiomas:
        db.add(
            CategoriaTraduccion(
...
def test_categoria_sin_traduccion_se_omite_solo_en_ese_idioma(db_session):
    ...
    assert "facturacion" in _ids(ensamblar_contenido(db_session, "es")["categorias"])
    assert "facturacion" not in _ids(ensamblar_contenido(db_session, "pt")["categorias"])
...
def test_la_categoria_a_medias_no_arrastra_a_las_completas(db_session):
    ...
    assert "cuenta" in _ids(ensamblar_contenido(db_session, "pt")["categorias"])
...
def test_articulo_sin_traduccion_se_omite_solo_en_ese_idioma(db_session):
    ...
    assert _ids(ensamblar_contenido(db_session, "es")["articulos"]) == [
        "solo-en-espanol",
        "bilingue",
    ]
    assert _ids(ensamblar_contenido(db_session, "pt")["articulos"]) == ["bilingue"]
...
def test_conversacion_ausente_devuelve_lista_vacia(db_session):
    db_session.delete(db_session.get(Conversacion, "pt"))
    db_session.commit()

    contenido = ensamblar_contenido(db_session, "pt")
...
def test_base_sin_metricas_devuelve_lista_vacia(db_session):
    ...
    assert ensamblar_contenido(db_session, "es")["metricas"] == []
...
def test_las_categorias_salen_por_su_campo_orden(db_session):
    ...
    assert _ids(ensamblar_contenido(db_session, "es")["categorias"]) == [
        "primera",
        "cuenta",
        "ultima",
    ]
...
def test_las_metricas_salen_por_su_campo_orden(db_session):
    db_session.add(Metrica(idioma="es", clave="resueltas", valor="120", orden=-1))
    db_session.commit()

    claves = [m["clave"] for m in ensamblar_contenido(db_session, "es")["metricas"]]
    assert claves == ["resueltas", "sinResolver"]


def test_las_metricas_de_un_idioma_no_se_mezclan_con_las_del_otro(db_session):
    db_session.add(Metrica(idioma="pt", clave="soloPt", valor="7", orden=1))
    db_session.commit()

    assert [m["clave"] for m in ensamblar_contenido(db_session, "es")["metricas"]] == ["sinResolver"]
    assert [m["clave"] for m in ensamblar_contenido(db_session, "pt")["metricas"]] == [
        "sinResolver",
        "soloPt",
    ]
...
def test_la_fecha_del_articulo_sale_en_iso(db_session):
    ...
    assert ensamblar_contenido(db_session, "es")["articulos"][0]["actualizado"] == "2026-07-25"
...
def test_el_id_de_categoria_del_articulo_es_el_estable_no_el_slug(db_session):
    ...
    es = ensamblar_contenido(db_session, "es")["articulos"][0]
    pt = ensamblar_contenido(db_session, "pt")["articulos"][0]
