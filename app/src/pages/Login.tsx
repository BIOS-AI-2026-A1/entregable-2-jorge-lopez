...
import { guardarToken } from '@/auth/sesion'
import { rutas } from '@/i18n/rutas'
import { Ic } from '@/components/iconos'

export function Login({ idioma }: { idioma: Idioma }) {
  ...
  async function alEnviar(e: FormEvent) {
    ...
    try {
      ...
      const datos = (await resp.json()) as { access_token: string }
      guardarToken(datos.access_token)
      navigate(rutas.panel(idioma), { replace: true })
    } catch {
